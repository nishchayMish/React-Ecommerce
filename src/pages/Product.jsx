...
const Product = () => {
  ...
  return productData ? (
    <div className='border-t-2 pt-10 transistion-opacity ease-in duration-500 opacity-100'>
              ...
              <div className='flex gap-2'>
                {
                  productData.sizes.map((item,index)=>(
                    <button onClick={()=>setSize(item)}
                      className={`border py-2 px-4 cursor-pointer bg-gray-100 ${item === size ? 'border-orange-500' : ''}`}
                      key={index}>
                      {item}
                    </button>
                  ))
      ...
      {/* desc nd revies */}
      <div className='mt-20 shadow-lg border-t border-gray-100'>
          <div className='flex border-t border-gray-300'>
            <b className='shadow-lg px-5 py-3 text-md'>Description</b>
            <p className='shadow-lg px-5 py-3 text-md'>Reviews(122)</p>
          </div>
          <div className='flex flex-col gap-4 px-6 py-6 text-sm text-gray-500 shadow-lg '>
            <p>
